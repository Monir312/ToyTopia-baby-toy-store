...
const Navbar = () => {
  ...
  return (
<div className="w-full bg-gradient-to-r from-pink-100 via-purple-100 to-blue-100 
                px-4 md:px-12 shadow-md fixed top-0 left-0 z-50 backdrop-blur-md">



      {/* Entire navbar content wrapped inside max-w-6xl */}
      <div className="max-w-[88vw] mx-auto navbar">
        ...
        <div className="navbar-start">
          ...
          <Link to="/" className="flex items-center gap-2">
            <img src={logoImg} alt="ToyTopia Logo" className="w-8 h-8" />
        ...
        <div className="navbar-end flex items-center gap-3">
          {user ? (
            <>
              <div data-tooltip-id="user-tooltip" data-tooltip-content={user?.displayName || "User"}>
                {user.photoURL ? (
                  <Link to='/profile' className="cursor-pointer">
                    <img src={user.photoURL} alt="user" className="w-10 h-10 rounded-full border-2 border-pink-400 hover:scale-105 transition" />
                  </Link>
                ) : (
              ...
              <button onClick={handleLogout} className="btn btn-sm bg-pink-500 hover:bg-pink-600 text-white">
                Logout
              </button>
            </>
          ) : (
            <Link to="/auth/login" className="btn btn-sm bg-pink-500 hover:bg-pink-600 text-white">Login</Link>
          )}
        </div>
